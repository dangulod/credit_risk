...
public:
    ...
    inline int minimize(type T::*f, T1& obj, Vector& x, Scalar& fx, const Vector& lb, const Vector& ub, Args&&... args)
    {
        using std::abs;
        ...
        LineSearch<Scalar>::gradient(f, obj, x, m_grad, std::forward<Args>(args)...);
        ...
        for( ; ; )
        {
            ...
            LineSearch<Scalar>::LineSearch(f, obj, fx, x, m_grad, step, step_max, m_drt, m_xp, m_param, std::forward<Args>(args)...);
...
template <class type, class T, class T1, class... Args>
auto L_BFGS_B(type T::*f, T1& obj, Vector& x, double &fx, Vector & lb, Vector ub, Args&&... args)
{
    LBFGSBParam<Scalar> param;
    LBFGSBSolver<Scalar> solver(param);

    return solver.minimize(f, obj, x, fx, lb, ub, std::forward<Args>(args)...);
}
