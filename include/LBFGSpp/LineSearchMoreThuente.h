...
public:
    ...
    static void gradient(type T::*f, T1& obj, Vector& x, Vector& grad, Args&&... args)
    {
        double val1, val2;
        Vector x0 = x;

        for (long jj = 0; jj < grad.size(); jj++)
        {
            x0[jj] = x[jj] + 1e-6;
            val1 = (static_cast<T &>(obj).*f)(x0, std::forward<Args>(args)...);

            x0[jj] = x[jj] - 1e-6;
            val2 = (static_cast<T &>(obj).*f)(x0, std::forward<Args>(args)...);

            x0[jj] = x[jj];

            grad[jj] = (val1 - val2) / (2 * 1e-6);
    ...
    ///
    template <class type, class T, class T1, class... Args>
    static void LineSearch(type T::*f, T1& obj, Scalar& fx, Vector& x, Vector& grad,
                           Scalar& step, const Scalar& step_max,
                           const Vector& drt, const Vector& xp,
                           const LBFGSBParam<Scalar>& param, Args&&... args)
    {
        ...
        gradient(f, obj, x, grad, std::forward<Args>(args)...);
